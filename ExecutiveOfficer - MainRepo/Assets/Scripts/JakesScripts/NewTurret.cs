...
public class NewTurret : MonoBehaviour {
    ...
    private void Init() {
        isLoaded = true;
        turretAudio = GetComponentInParent<AudioSource>();
    ...
    private void LaunchProjectile() {
        //Adding spread
        Vector3 originalRotation = barrelEnd.localEulerAngles;
        Vector3 shotRotation;
        shotRotation = new Vector3(originalRotation.x + (Random.Range(-spread, spread)), originalRotation.y + (Random.Range(-spread, spread)), originalRotation.z);
        barrelEnd.rotation = Quaternion.Euler(shotRotation);
        ...
        turretAudio.PlayOneShot(shotAudio);
    }
